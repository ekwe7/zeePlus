...
export const AdminDashboard = () => {
  ...
  const activeStaff = users.filter((u) => u.role === "doctor" || u.role === "pharmacist");

  return (
    <div className="space-y-6">
      <PageHeader title="Admin Dashboard" subtitle="Overview of hospital operations and users." />

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Patients" value={users.filter((u) => u.role === "patient").length} />
        <StatCard label="Active Staff" value={activeStaff.length} />
        <StatCard label="Total Appointments" value={appointments.length} />
        <StatCard label="Pending Claims" value={claims.filter((c) => c.status === "PENDING").length} />
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <Badge variant="default" className="bg-green-600">Idle</Badge>
              </div>
            </div>
          </CardContent>
        </Card>
